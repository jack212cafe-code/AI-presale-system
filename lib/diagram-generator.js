...
const DIAGRAM_TEXT_FORMAT = {
  ...
  schema: {
    ...
    properties: {
      ...
      thai_explanation: {
        type: "string",
        description: "Thai prose explaining the topology: architecture overview, bandwidth design rationale, redundancy strategy, and backup data flow"
      }
    },
    required: ["devices", "connections", "thai_explanation"]
  }
};
  ...
  const { output, mock } = await generateJsonWithOpenAI({
    ...
    mockResponseFactory: async () => ({
      devices: [
        ...
        { id: "SRV1", label: "PowerEdge R760 x3", type: "server" },
        { id: "STOR", label: "PowerStore 500T", type: "storage" },
        { id: "BU", label: "PowerProtect DD6400", type: "backup" },
      ...
      connections: [
        ...
        { from: "SRV1", to: "STOR", label: "NVMe/25GbE" },
        { from: "SRV1", to: "SW2", label: "10GbE" },
        { from: "SW2", to: "BU", label: "10GbE" },
        { from: "SW1", to: "CLI", label: "1GbE" }
      ],
      thai_explanation: "Topology นี้ออกแบบเป็น 3-tier แบบ classic ที่ compute (HCI nodes) แยกจาก storage โดยใช้ All-flash NVMe สำหรับ primary workload ที่ต้องการ latency ต่ำ ส่วน backup แยกผ่าน dedicated switch เพื่อไม่ให้ conflict กับ production traffic ทั้งระบบใช้ 25GbE สำหรับ server-to-storage เพื่อรองรับ VM density ที่สูง (20+ VMs) และมี redundancy N+1 จาก 3-node cluster"
    })
  });

  const mermaidCode = buildMermaidCode(output.devices, output.connections);
  return { mermaidCode, thaiExplanation: output.thai_explanation, mock };
}