...
export function normalizeKnowledgeUploadPayload(payload) {
  ...
  assert(ALLOWED_EXTENSIONS.has(extension), `Unsupported file type: ${extension}. Allowed: ${[...ALLOWED_EXTENSIONS].join(", ")}`);

  const b64 = String(payload.content_base64).replace(/^data:[^;]+;base64,/, "");
  const padding = (b64.match(/=+$/) || [""])[0].length;
  const approxBytes = Math.floor((b64.length * 3) / 4) - padding;
  assert(
    approxBytes <= MAX_UPLOAD_BYTES,
    `File too large: ${(approxBytes / 1024 / 1024).toFixed(1)}MB exceeds 15MB limit`
  );
