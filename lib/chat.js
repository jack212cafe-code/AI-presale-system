...
async function handleDiscoveryQuestions({ conversation, conversationId, message, userId, onProgress }) {
  return withTimeout(async () => {
    ...
    const specialistBriefs = await runAllSpecialists(requirements, { projectId: conversation.project_id, orgId: conversation.org_id ?? null });
    ...
    const solution = await runSolutionAgent(requirements, { projectId: conversation.project_id, specialistBriefs, orgId: conversation.org_id ?? null });
...
async function handleAwaitingSelection({ conversation, conversationId, message, onProgress }) {
  return withTimeout(async () => {
    ...
    if (vendorPreference) {
      ...
      if (requirements) {
        const specialistBriefs = await runAllSpecialists(requirements, { projectId: conversation.project_id, orgId: conversation.org_id ?? null });
        const newSolution = await runSolutionAgent(requirements, { projectId: conversation.project_id, specialistBriefs, orgId: conversation.org_id ?? null });
