...
import { createProjectRecord, getProjectById, persistRequirementsJson, persistSolutionJson, persistSpecialistBriefs, persistBomJson, approveProject, updateProjectName } from "./projects.js";
...
function formatPresaleBom(bom) {
  ...
  rows.forEach((row) => {
    ...
    const normalizedCategory = sections.find((section) => category.includes(section.title.replace(/[\[\]]/g, "")));
  ...
  return withTimeout(async () => {
    ...
    const saveAssistantState = async () => {
      ...
      await Promise.all([
        ...
        settle(updateConversationStage(conversationId, "bom"), "updateConversationStage"),
