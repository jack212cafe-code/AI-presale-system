...
async function handleDiscoveryQuestions({ conversation, conversationId, message, userId, onProgress }) {
  return withTimeout(async () => {
    ...
    if (!/(nutanix)/.test([intake?.primary_use_case, intake?.notes, message].join(" ").toLowerCase())) {
      ...
      // Also block from options since customer didn't explicitly ask for it
      if (!vendorPrefs.disliked.includes("Nutanix")) vendorPrefs.disliked.push("Nutanix");
    }
...
export async function handleChatMessage({ conversationId, message, userId, orgId, onProgress }) {
  ...
  if (orgId !== null && conversation.org_id !== orgId) {
    return {
      conversation_id: conversationId,
      project_id: null,
      stage: "error",
