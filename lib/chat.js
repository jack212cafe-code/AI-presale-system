...
import { createProjectRecord, getProjectById, persistRequirementsJson, persistSolutionJson, persistSpecialistBriefs, persistBomJson, approveProject, updateProjectName, listProjectsByCustomerName, getRejectedOptionsByCustomer } from "./projects.js";
...
function formatBomAndProposal(bom, project) {
  ...
  if (rows.length > 0) {
    rows.forEach((row) => {
      const cat = escMd(row.category);
      const desc = escMd(row.description);
      const qty = row.qty ?? 1;
      const notes = escMd(row.notes);
...
async function handleDiscoveryQuestions({ conversation, conversationId, message, userId, onProgress }) {
  return withTimeout(async () => {
    ...
    const VENDOR_KEYWORDS = {
      dell: "Dell", hpe: "HPE", nutanix: "Nutanix", cisco: "Cisco",
      veeam: "Veeam", proxmox: "Proxmox", vmware: "VMware",
    ...
    // Remove "Nutanix" from preferred if user didn't explicitly mention it
    // (Nutanix in searchText could come from prior constraints/context)
    if (!/(nutanix)/.test([intake?.primary_use_case, intake?.notes, message].join(" ").toLowerCase())) {
      vendorPrefs.preferred = vendorPrefs.preferred.filter(v => v !== "Nutanix");
    }
...
async function handleAwaitingSelection({ conversation, conversationId, message, onProgress }) {
  return withTimeout(async () => {
    ...
    const chatBudgetWarning = checkBudgetOverrun(
      solutionWithSelection.options?.[solutionWithSelection.selected_option ?? 0]?.estimated_tco_thb,
      project.requirements_json?.budget_range
    );
    const hasBlockers = groundingWarnings.length > 0 || !!chatBudgetWarning;
    ...
    const responseText = formatBomAndProposal(cleanBom, { status: "bom_complete" }) + groundingNotice + budgetNotice;

    // Save to DB with 10s timeout each — if Supabase slow, skip save but still return BOM to user
    const sbSave = (p) => Promise.race([p, new Promise(r => setTimeout(r, 10_000))]);
    onProgress?.(2, 4, "กำลังบันทึก BOM...");
    await sbSave(persistBomJson(conversation.project_id, bom));
    await sbSave(updateConversationStage(conversationId, "bom"));
    await sbSave(addMessage(conversationId, "assistant", responseText));

    // Run proposal in background — does NOT block user response
    if (!hasBlockers) {
      ...
      setImmediate(async () => {
        try {
          ...
          await runProposalAgent(proposalIntake, project.requirements_json, solutionWithSelection, bom, { projectId, budgetWarning: chatBudgetWarning });
          await updateConversationStage(conversationId, "complete");
          console.log(`[proposal:bg] done project=${projectId}`);
        } catch (err) {
          console.error(`[proposal:bg] failed project=${projectId}: ${err.message}`);
        }
      });
    }
...
async function handleFreeformQA({ conversation, conversationId, message }) {
  return withTimeout(async () => {
    ...
    const [history, kbChunks] = await Promise.all([
...
export async function handleChatMessage({ conversationId, message, userId, onProgress }) {
  ...
  if (!conversation) {
    return {
      ...
      project_id: null,
      stage: "error",
      text: "ไม่พบ conversation — กรุณาเริ่ม conversation ใหม่",
      created: false,
      ok: false
    };
  }
