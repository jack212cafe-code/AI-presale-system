...
async function handleAwaitingSelection({ conversation, conversationId, message, onProgress }) {
  return withTimeout(async () => {
    ...
    console.log(`[chat:bom] starting BOM for project=${conversation.project_id}`);
    ...
    console.log(`[chat:bom] BOM done rows=${bom?.rows?.length ?? 0}`);
    onProgress?.(2, 4, "กำลังบันทึก BOM...");
    await persistBomJson(conversation.project_id, bom);
    console.log("[chat:bom] BOM persisted, updating stage");
    await updateConversationStage(conversationId, "bom");
    console.log("[chat:bom] stage updated to bom");

    // Gate 2a: GROUNDING WARNING check
    const groundingWarnings = extractBomGroundingWarnings(bom);

    // Gate 2b: Budget overrun check
    ...
    // Only approve if no grounding warnings and no budget overrun
    ...
    if (!hasBlockers) {
      onProgress?.(3, 4, "กำลังเขียน proposal...");
      ...
      onProgress?.(3, 4, "⚠️ พบข้อสังเกต — รอการตรวจสอบ");
    }

    await updateConversationStage(conversationId, hasBlockers ? "bom" : "complete");
    ...
    const responseText = formatBomAndProposal(cleanBom, finalProject) + groundingNotice + budgetNotice;
    await addMessage(conversationId, "assistant", responseText);
