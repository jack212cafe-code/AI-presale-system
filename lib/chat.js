...
function detectRevisionIntent(message) {
  ...
  if (/แก้ไข|revision|ปรับ|modify|update|เปลี่ยน/.test(lower)) return "vendor";
  return null;
}
...
async function handleRevision({ conversation, conversationId, message, userId, intent }) {
  return withTimeout(async () => {
    ...
    const requirements = project?.requirements_json
      ? { ...project.requirements_json, constraints: [...(project.requirements_json.constraints ?? []), `Revision request: ${message}`] }
      : null;
...
export async function handleChatMessage({ conversationId, message, userId }) {
  ...
  if (stage === "complete") {
    ...
    return {
      ...
      project_id: conversation.project_id,
      stage: "complete",
      text: "Proposal สร้างเสร็จแล้วครับ หากต้องการแก้ไข กด 'เปลี่ยน vendor', 'เปลี่ยน solution' หรือ 'ปรับ requirements' ได้เลยครับ",
      created: false
    };
  }
