...
export async function upsertKnowledgeBase(entries, orgId = null) {
  ...
  const records = entries.map((entry) => ({
    ...entry,
    org_id: entry.org_id ?? orgId ?? null
  }));
...
export async function listKnowledgeDocuments(orgId = null) {
  const client = getSupabaseAdmin();
  if (!client) {
    return [];
  }
...
export async function retrieveKnowledgeFromVector(queryEmbedding, matchCount = 5, orgId = null) {
  if (!getSupabaseAdmin()) {
    return [];
  }
  ...
  let fallbackUrl = `${config.supabase.url}/rest/v1/knowledge_base?select=id,source_key,category,title,content,metadata,embedding&embedding=not.is.null`;
  if (orgId !== null) {
    fallbackUrl += `&or=(org_id.is.null,org_id.eq.${orgId})`;
  }

  const kbResponse = await fetchWithTimeout(fallbackUrl, {
    headers: {
      "apikey": config.supabase.serviceRoleKey,
      "Authorization": `Bearer ${config.supabase.serviceRoleKey}`
    }
...
export async function retrieveKnowledgeByVendorFilter(vendorNames, chunksPerVendor = 4, orgId = null) {
  return withDbTimeout(_retrieveKnowledgeByVendorFilter(vendorNames, chunksPerVendor, orgId), 20_000, "retrieveKnowledgeByVendorFilter");
}

async function _retrieveKnowledgeByVendorFilter(vendorNames, chunksPerVendor = 4, orgId = null) {
  const client = getSupabaseAdmin();
  ...
  for (const vendor of vendorNames) {
    ...
    for (const kw of keywords) {
      ...
      let fileQuery = client.from("knowledge_base")
        .select("source_key")
        .ilike("source_key", `%${kw}%`)
        .not("content", "ilike", "-- % of %");
      if (orgId !== null) {
        fileQuery = fileQuery.or(`org_id.is.null,org_id.eq.${orgId}`);
      }
      const { data: fileRows } = await fileQuery.order("source_key", { ascending: true });
  ...
  await Promise.all(sourceFiles.map(async (file) => {
    let chunkQuery = client.from("knowledge_base")
      .select("source_key, title, content, category, metadata")
      .ilike("source_key", `${file}%`)
      .not("content", "ilike", "-- % of %");
    if (orgId !== null) {
      chunkQuery = chunkQuery.or(`org_id.is.null,org_id.eq.${orgId}`);
    }
