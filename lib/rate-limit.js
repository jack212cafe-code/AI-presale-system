...
export async function requireRateLimitDb(request, response, userId, bucket, tier = "growth") {
  const limits = config.rateLimit?.[bucket];
  if (!limits) return true;

  const multiplier = SAAS_TIERS[tier]?.rate_limit_multiplier || 1.0;
  const adjustedMax = Math.floor(limits.max * multiplier);
  const key = `${userId}:${bucket}`;

  const { checkLimitDb } = await import("./rate-limit-db.js");
  let result = await checkLimitDb(key, adjustedMax, limits.windowMs);
  ...
  if (result === null) {
    ...
    if (win.count > adjustedMax) {
      ...
      result = { allowed: false, retryAfterSec };
    } else {
      result = { allowed: true };
    }
  }
  ...
  if (!result.allowed) {
    ...
    response.end(JSON.stringify({
      ok: false,
      error: `Rate limit exceeded — ใช้งานได้สูงสุด ${adjustedMax} requests ต่อ ${Math.round(limits.windowMs / 60000)} นาที กรุณารอ ${result.retryAfterSec} วินาที`,
