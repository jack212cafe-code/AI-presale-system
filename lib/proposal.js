...
function reviewSignoffTable(date) {
  const colWidths = [30, 35, 35];
  const labels = ["บทบาท", "ชื่อ-นามสกุล", "วันที่ / ลายเซ็น"];
  const rows = [
    ["Presale Engineer", "____________________", "____________________"],
    ["Sales Manager", "____________________", "____________________"]
  ];
  ...
  const headerRow = new TableRow({
    ...
    children: labels.map((text, i) =>
      new TableCell({
        ...
        children: [new Paragraph({ children: [new TextRun({ text, bold: true, size: 18, font: "Calibri", color: "475569" })], spacing: { after: 0 } })]
      })
    )
  });
      ...
      children: cells.map((text, ci) =>
        new TableCell({
          ...
          children: [new Paragraph({
            children: [new TextRun({ text, size: 18, font: "Calibri", color: ci === 0 ? "1E293B" : "94A3B8" })],
            spacing: { after: 0 }
          })]
        })
      )
...
function coverBlock(customerName, projectName, date) {
  return [
    new Paragraph({
      children: [new TextRun({ text: "Franky-Presale", size: 18, color: "94A3B8", font: "Calibri", bold: true })],
      spacing: { after: 80 }
    }),
    new Paragraph({
...
export async function buildProposalBuffer({
  ...
  const children = [
    ...
    ...(solutionTopology ? [
      h2("โครงสร้าง (Topology)"),
      p(solutionTopology, { spacing: { after: 120 } })
    ] : []),

    ...(solutionHypervisor && solutionHypervisor !== "N/A" ? [
      h2("Hypervisor"),
      p(solutionHypervisor, { spacing: { after: 120 } })
    ] : []),
    ...
    h1("4. รายการอุปกรณ์ (Bill of Materials)"),
    new Paragraph({
    ...
    h1("7. การตรวจสอบและอนุมัติ"),
    new Paragraph({
      children: [new TextRun({ text: "เอกสารนี้ต้องได้รับการตรวจสอบจาก presale engineer ก่อนส่งให้ลูกค้า", size: 20, font: "Calibri", color: "475569" })],
      spacing: { after: 200 }
    }),
    reviewSignoffTable(date),

    divider(),
    new Paragraph({
      children: [
        new TextRun({ text: "CONFIDENTIAL", bold: true, size: 16, color: "94A3B8", font: "Calibri", allCaps: true }),
        new TextRun({ text: " — เอกสารนี้จัดทำเพื่อใช้ภายในองค์กรเท่านั้น ห้ามเผยแพร่โดยไม่ได้รับอนุญาต", size: 16, italics: true, color: "94A3B8", font: "Calibri" })
      ],
  ...
  const document = new Document({
    title: projectName || `${customerName} Presale Proposal`,
    description: `Presale proposal for ${customerName}`,
    ...
    sections: [{
      ...
      headers: {
        default: new Header({
          children: [new Paragraph({
            children: [
              new TextRun({ text: `${customerName} — Presale Proposal`, size: 14, color: "94A3B8", font: "Calibri" })
            ],
      ...
      footers: {
        default: new Footer({
          children: [new Paragraph({
            children: [
              new TextRun({ text: "Confidential | Franky-Presale | Page ", size: 14, color: "94A3B8", font: "Calibri" }),
