...
export async function createUserSession(userId, displayName, role = "engineer") {
  ...
  if (client) {
    try {
      const { data, error } = await client
        ...
        .single();
      if (error) {
        logger.warn("session.fetch_org_id_failed", { userId, error: error.message });
      } else {
        orgId = data?.org_id ?? null;
      }
    } catch (err) {
      logger.warn("session.fetch_org_id_error", { userId, error: err.message });
    }
  }
...
export async function loadPersistedSessions() {
  ...
  const { data, error } = await client
    .from("sessions")
    .select("token, user_id, display_name, role, org_id, created_at, expires_at")
    .gt("expires_at", now);
  if (error) {
...
export async function validateUserCredentials(username, password) {
  ...
  if (!client) {
    ...
    return { id: local.username, username: local.username, display_name: local.display_name, role: local.role ?? "engineer", org_id: null };
  }

  const { data, error } = await client
    .from("users")
    .select("id, username, password_hash, display_name, role, org_id")
  ...
  return { id: data.id, username: data.username, display_name: data.display_name, role: data.role ?? "engineer", org_id: data.org_id };
}
...
export async function ensureUsersSeeded() {
  ...
  for (const user of LOCAL_USERS) {
    const password_hash = user.password_hash
      ?? (user._plain_password ? await bcrypt.hash(user._plain_password, 10) : null);
    await client
      .from("users")
      .upsert(
        { username: user.username, password_hash, display_name: user.display_name, role: user.role ?? "engineer" },
        { onConflict: "username" }
      )
      .then(({ error }) => {
